...
/// Int Affine Coordinate
typedef int32_t IAffPoint4[4];
/// Float Affine Coordinate
...
/// Unitize: P = (P[0]/P[3], P[1]/P[3], P[2]/P[3], 1)
void floatAffineUnitize(FAffPoint4 P);
/// Unitize: d = A - B = BA
void floatAffineDiff(const FAffPoint4 A, const FAffPoint4 B, FVec4 d);
/**
 * Reflect: The mirror point of P determined by the plane passing through O and orthogonal to d
 *              P = P - 2(OP·d)/sqrt(d·d) * d
 */
void floatAffineReflect(FAffPoint4 P, const FAffPoint4 O, const FVec4 d);
/**
 * Reflect: The flip point of P with respect to O about d
 *              P = O + 2(OP·d)/sqrt(d·d) * d - OP
 */
